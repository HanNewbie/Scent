...
@section('content')
    <div x-data="{ open: false }">
        ...
        <div id="section-favorit" class="hidden mx-4 sm:mx-10 lg:mx-[75px]"
            x-data="productModal({{ $favProducts->toJson() }})">
            ...
            <div class="max-w-7xl mx-auto px-4 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-4">
                @forelse ($favorites as $fav)
                    ...
                    <div
                        ...
                        <span
                            class="absolute top-2 right-12 px-3 py-1 rounded-full text-xs font-medium
                        ...
                        <form action="{{ route('user.favorites.remove', $product->id) }}" method="POST"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="bg-white rounded-full w-9 h-9 flex items-center justify-center shadow hover:shadow-lg transition">
                        ...
                        <div class="p-4">
                            ...
                            <p class="text-amber-900 font-semibold text-xl">
                                Rp {{ number_format($product->display_price, 0, ',', '.') }}
                            </p>

                            @if ($isWebUser && !$isAdmin)
                                <button type="button" @click="openProduct({{ $product->id }})"
                                    class="w-full mt-3 bg-amber-800 hover:bg-amber-700 text-white font-medium py-2 px-4 rounded-lg transition
                            ...
                            @elseif(!$isWebUser && !$isAdmin)
                                <a href="{{ route('login') }}">
                                    <button type="button"
                                        class="w-full mt-3 bg-amber-800 hover:bg-amber-700 text-white font-medium py-2 px-4 rounded-lg transition duration-300
                                        {{ $product->display_stok <= 0 ? 'opacity-50 cursor-not-allowed hover:bg-amber-800' : '' }}"
                                        {{ $product->display_stok <= 0 ? 'disabled' : '' }}>
                                        <i class="fa fa-cart-plus"></i>
                                        Tambahkan ke Keranjang
